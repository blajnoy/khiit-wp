...
final class WP_Fullpage extends WP_Fullpage_Base {
	...
	public function scripts_styles() {
		...
		$fullpage_custom_script_url = $this->convert_path_to_uri( $fullpage_custom_script_path );

		wp_enqueue_script( 'jquery-fullpage-custom', $fullpage_custom_script_url, array( 'jquery-fullpage', 'jquery-pseudo' ), WPFP_VERSION );
		
		// Get the path to 'jquery.fullpage.theme.js'.
		// See if the file exists in the theme
		$fullpage_theme_script_path = $this->locate_template(
			trailingslashit( $this->script_path() ) . 'jquery.fullpage.theme.js'
		);

		// Convert the previous script path to an URL
		$fullpage_theme_script_url = $this->convert_path_to_uri( $fullpage_theme_script_path );

		wp_enqueue_script( 'jquery-fullpage-theme', $fullpage_theme_script_url, array( 'jquery-fullpage-custom' ), WPFP_VERSION );
		wp_enqueue_script( 'jquery-fullpage-init', $this->assets_url . 'js/jquery.fullPage.init.js', array( 'jquery-fullpage-theme' ), WPFP_VERSION );
		...
		wp_enqueue_style( 'jquery-fullPage', $this->assets_url . 'css/jquery.fullPage.css', array( 'materialize' ), WPFP_VERSION );
		...
		$fullpage_custom_style_url = $this->convert_path_to_uri( $fullpage_custom_style_path );
		
		wp_enqueue_style( 'jquery-fullPage-custom', $fullpage_custom_style_url, array( 'jquery-fullPage', 'dashicons' ), WPFP_VERSION );
		
		// Get the path to 'jquery.fullpage.theme.css'.
		// See if the file exists in the theme
		$fullpage_theme_style_path = $this->locate_template(
			trailingslashit( $this->style_path() ) . 'jquery.fullpage.theme.css'
		);

		// Convert the previous style path to an URL
		$fullpage_theme_style_url = $this->convert_path_to_uri( $fullpage_theme_style_path );
		
		wp_enqueue_style( 'jquery-fullPage-theme', $fullpage_theme_style_url, array( 'jquery-fullPage-custom' ), WPFP_VERSION );
	 ...
	 * @return  string
	 */
	public function convert_path_to_uri( $path ) {
	...
	} // END public function convert_path_to_uri
	...
	public function get_loop( $type, $name = '' ) {
		...
		while ( WPFP_Query()->$have_types() ) {
			...
			if( $template ) {
				...
				if( $type === 'slide' ) WPFP_Query()->get_dynamic_slide_style();
				
				if( $type === 'section' ) WPFP_Query()->get_dynamic_section_style();

			}

		}

		if( $type === 'section' ) WPFP_Query()->get_dynamic_fullpage_style();
