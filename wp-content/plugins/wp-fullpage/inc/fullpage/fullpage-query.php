...
final class WP_Fullpage_Query {
	 ...
	 * @return  bool
	 */
	public function fullpage_has_full_navigation() {

		$fullpage_options = $this->fullpage->fullpage_options;

		$full_navigation = 'no';

		if( ! empty( $fullpage_options['fullNavigation'] ) )
			$full_navigation = $fullpage_options['fullNavigation'];
		
		return $full_navigation === 'yes' ? true : false;

	} // END public function fullpage_has_full_navigation
	...
	public function get_slide_position( $section_index = -1, $slide_index = -1, $print = true ) {
		...
		if( empty( $vertical_position ) || 'inherit' === $vertical_position )
	 ...
	 * @return  string
	 */
	public function get_content_margin( $section_index = -1, $slide_index = -1, $print = true ) {
		...
		$contentWidth = (int) $this->get_slide_option( $section_index, $slide_index, 'contentWidth', 50 );
		$positions    = explode( ' ', $this->get_slide_position( $section_index, $slide_index, false) );
		$margin_left  = '';
		$margin_right = '';
		...
		switch ( $positions[1] ) {
			case 'left':
				$margin_left  = 0;
				$margin_right = 100 - $contentWidth;
				break;
			
			case 'right':
				$margin_right = 0;
				$margin_left  = 100 - $contentWidth;
				break;
			
			default:
				$margin_right = $margin_left  = ( 100 - $contentWidth ) / 2;
				break;
		}

		$margin = sprintf( "margin-left: %1s%%; margin-right: %2s%%;", $margin_left, $margin_right );
	...
	} // END public function get_content_margin
	...
	public function get_slide_color( $section_index = -1, $slide_index = -1, $print = true ) {
		...
		$color = $this->get_slide_option( $section_index, $slide_index, 'slideColor', 'transparent' );
	...
	public function get_fullpage_easing( $print = true ) {
		...
		if( ! empty( $fulpage_options['easingCss3'] ) )
	...
	public function get_section_easing( $section_index = -1, $print = true ) {
		...
		// the easing option of the fullpage
		if( ( empty( $easing ) || 'inherit' === $easing ) && ! empty( $fulpage_options['easingCss3'] ) )
			$easing = $fulpage_options['easingCss3'];
	 ...
	 * @return  bool 		
	 */
	public function display_section_title( $section_index = -1, $slide_index = -1 ) {
		...
		// the slide option
		if( ! empty( $slide_options['displaySectionTitle'] ) )
			$displaySectionTitle = $slide_options['displaySectionTitle'];

		// the slide option of the section
		if( ( empty( $displaySectionTitle ) || 'inherit' === $displaySectionTitle ) && ! empty( $section_options['displaySectionTitle'] ) )
			$displaySectionTitle = $section_options['displaySectionTitle'];
		
		// the slide option of the fullpage
		if( ( empty( $displaySectionTitle ) || 'inherit' === $displaySectionTitle ) && ! empty( $fulpage_options['displaySectionTitle'] ) )
			$displaySectionTitle = $fulpage_options['displaySectionTitle'];
		
		return empty( $displaySectionTitle ) ? false : ( $displaySectionTitle === 'yes' ? true : false );

	} // END public function display_section_title
	 ...
	 * @return  bool 		
	 */
	public function display_title( $section_index = -1, $slide_index = -1 ) {
		...
		// the slide option
		if( ! empty( $slide_options['displayTitle'] ) )
			$displayTitle = $slide_options['displayTitle'];

		// the slide option of the section
		if( ( empty( $displayTitle ) || 'inherit' === $displayTitle ) && ! empty( $section_options['displayTitle'] ) )
			$displayTitle = $section_options['displayTitle'];
		
		// the slide option of the fullpage
		if( ( empty( $displayTitle ) || 'inherit' === $displayTitle ) && ! empty( $fulpage_options['displayTitle'] ) )
			$displayTitle = $fulpage_options['displayTitle'];
		
		return empty( $displayTitle ) ? true : ( $displayTitle === 'yes' ? true : false );

	} // END public function display_title
	 ...
	 * @return  bool 		
	 */
	public function display_date( $section_index = -1, $slide_index = -1 ) {
		...
		// the slide option
		if( ! empty( $slide_options['displayDate'] ) )
			$displayDate = $slide_options['displayDate'];

		// the slide option of the section
		if( ( empty( $displayDate ) || 'inherit' === $displayDate ) && ! empty( $section_options['displayDate'] ) )
			$displayDate = $section_options['displayDate'];
		
		// the slide option of the fullpage
		if( ( empty( $displayDate ) || 'inherit' === $displayDate ) && ! empty( $fulpage_options['displayDate'] ) )
			$displayDate = $fulpage_options['displayDate'];
		
		return empty( $displayDate ) ? false : ( $displayDate === 'yes' ? true : false );

	} // END public function display_date
	 ...
	 * @return  bool 		
	 */
	public function display_author( $section_index = -1, $slide_index = -1 ) {
		...
		// the slide option
		if( ! empty( $slide_options['displayAuthor'] ) )
			$displayAuthor = $slide_options['displayAuthor'];

		// the slide option of the section
		if( ( empty( $displayAuthor ) || 'inherit' === $displayAuthor ) && ! empty( $section_options['displayAuthor'] ) )
			$displayAuthor = $section_options['displayAuthor'];
		
		// the slide option of the fullpage
		if( ( empty( $displayAuthor ) || 'inherit' === $displayAuthor ) && ! empty( $fulpage_options['displayAuthor'] ) )
			$displayAuthor = $fulpage_options['displayAuthor'];
		
		return empty( $displayAuthor ) ? false : ( $displayAuthor === 'yes' ? true : false );

	} // END public function display_author
	 ...
	 * @return  string
	 */
	public function auto_height( $section_index = -1, $print = true ) {
		...
		// the easing option of the section
		if( ! empty( $section_options['autoHeight'] ) )
			$auto_height = $section_options['autoHeight'] === 'yes' ? 'fp-auto-height' : '';
	...
	} // END public function auto_height
	 ...
	 * @return  string
	 */
	public function get_fullpage_option( $option_name, $default_value ) {

		$fullpage_options = $this->fullpage->fullpage_options;
		
		// the option of the fullpage
		if( ! empty( $fullpage_options[ $option_name ] ) ) {
			return $fullpage_options[ $option_name ];
		}

		// the default option
		return $default_value;

	} // END public function get_section_option

	/**
	 * Retrieve section option.
	 * 
	 * @param   int      	$section_index  	The section index. If empty, will take the current section.
	 * @param   string      $option_name
	 * @param   string      $default_value
	 * 
	 * @return  string
	 */
	public function get_section_option( $section_index, $option_name, $default_value ) {
		
		$section_options  = $this->sections[ $section_index ]->section_options;
		$fullpage_options = $this->fullpage->sections_options;

		// the option of the section
		if( ! empty( $section_options[ $option_name ] ) ) {
			return $section_options[ $option_name ];
		}
	 ...
	 * 
	 * @param   int      	$section_index  	The section index. If empty, will take the current section.
	 * @param   int      	$slide_index  	    The slide index. If empty, will take the current slide.
	 * @param   string      $option_name
	 * @param   string      $default_value
	 * 
	 * @return  string
	 */
	public function get_slide_option( $section_index, $slide_index, $option_name, $default_value ) {

		$slide_options    = $this->sections[ $section_index ]->slides[ $slide_index ]->slide_options;
		$section_options  = $this->sections[ $section_index ]->slides_options;
		$fullpage_options = $this->fullpage->slides_options;

		// the option of the slide
		if( ! empty( $slide_options[ $option_name ] ) ) {
			return $slide_options[ $option_name ];
		}
	...
	} // END public function get_slide_option
	 ...
	 * @return  void
	 */
	public function get_dynamic_slide_style( $section_index = -1, $slide_index = -1 ) {
		...
		$slide   = $this->sections[ $section_index ]->slides[ $slide_index ];
		$section = $this->sections[ $section_index ];

		$slideColor             = $this->get_slide_option( $section_index, $slide_index, 'slideColor', 'transparent' );
		$contentWidth           = $this->get_slide_option( $section_index, $slide_index, 'contentWidth', 50 ); 
		$contentInnerHMargin    = $this->get_slide_option( $section_index, $slide_index, 'contentInnerHMargin', 0 );
		$contentInnerVMargin    = $this->get_slide_option( $section_index, $slide_index, 'contentInnerVMargin', 0 );
		$contentBackgroundColor = $this->get_slide_option( $section_index, $slide_index, 'contentBackgroundColor', 'transparent' );
		$contentStyle           = $this->get_slide_option( $section_index, $slide_index, 'contentStyle', '' );
		$innerContentStyle      = $this->get_slide_option( $section_index, $slide_index, 'innerContentStyle', '' );
		
		WPFP_Helpers()->ob_start();

			?>

				.section.<?php print $section->post_name; ?> .slide.<?php print $slide->post_name; ?> {
					background-color: <?php print $slideColor; ?>;
				}

				.section.<?php print $section->post_name; ?> .slide.<?php print $slide->post_name; ?> article {
					width: <?php print $contentWidth; ?>%;
					<?php $this->get_content_margin( $section_index, $slide_index ); ?>
				}

				.section.<?php print $section->post_name; ?> .slide.<?php print $slide->post_name; ?> article div.inner-slide {
					margin: <?php print $contentInnerVMargin; ?>% <?php print $contentInnerHMargin; ?>%;
					background-color: <?php print $contentBackgroundColor; ?>;
				}

				#fullpage .section.<?php print $section->post_name; ?> .slide.<?php print $slide->post_name; ?> article {
					<?php print $contentStyle; ?>
				}

				#fullpage .section.<?php print $section->post_name; ?> .slide.<?php print $slide->post_name; ?> article div.inner-slide {
					<?php print $innerContentStyle; ?>
				}

			<?php

		WPFP_Helpers()->add_dynamic_css( WPFP_Helpers()->ob_get_clean() );

	} // END public function get_dynamic_slide_style
	 ...
	 * @return  void
	 */
	public function get_dynamic_fullpage_style() {
		
		$fullpage_options        = $this->fullpage->fullpage_options;

		$fullpageScrollingSpeed = ! empty( $fullpage_options['autoScrolling'] ) && 'no' === $fullpage_options['autoScrolling'] ? 10 : 
					$this->get_fullpage_option( 'scrollingSpeed', '700' );
		
		$sectionScrollingSpeed               = $this->get_fullpage_option( 'scrollingSpeed', '700' );
		$fullNavigationBackgroundColor       = $this->get_fullpage_option( 'fullNavigationBackgroundColor', 'rgba(255, 255, 255, 0.8)' );
		$fullNavigationTextColor             = $this->get_fullpage_option( 'fullNavigationTextColor', '#666666' );
		$fullNavigationBackgroundColor       = $this->get_fullpage_option( 'fullNavigationBackgroundColor', 'rgba(255, 255, 255, 0.8)' );
		$fullNavigationActiveTextColor       = $this->get_fullpage_option( 'fullNavigationActiveTextColor', '#ffffff' );
		$fullNavigationActiveBackgroundColor = $this->get_fullpage_option( 'fullNavigationActiveBackgroundColor', 'rgba(130, 130, 130, 0.8)' );
		$sectionsNavigationColor             = $this->get_fullpage_option( 'sectionsNavigationColor', '#333333' );
		$sectionsNavigationActiveColor       = $this->get_fullpage_option( 'sectionsNavigationActiveColor', '#666666' );
		$tooltipColor                        = $this->get_fullpage_option( 'tooltipColor', '#ffffff' );
		$tooltipBgColor                      = $this->get_fullpage_option( 'tooltipBgColor', '#333333' );
		$fullpageColor                       = $this->get_fullpage_option( 'fullpageColor', 'transparent' );

		WPFP_Helpers()->ob_start();

			?>

				#wpfp-navmenu #wpfp-sidemenu {
					background-color: <?php print $fullNavigationBackgroundColor; ?>;
				}

				#wpfp-navmenu .wpfp-menu > li > a,
				#wpfp-navmenu .wpfp-menu > li li a {
					color: <?php print $fullNavigationTextColor; ?>;
					background-color: <?php print $fullNavigationBackgroundColor; ?>;
				}

				#wpfp-navmenu .wpfp-menu > li.active > a,
				#wpfp-navmenu .wpfp-menu > li > a:hover,
				#wpfp-navmenu .wpfp-menu > li li.active > a,
				#wpfp-navmenu .wpfp-menu > li li a:hover {
					color: <?php print $fullNavigationActiveTextColor; ?>;
					background-color: <?php print $fullNavigationActiveBackgroundColor; ?>;
				}

				#wpfp-navmenu a.button-collapse {
					color: <?php print $fullNavigationBackgroundColor; ?>;
				}

				#wpfp-navmenu a.button-collapse:hover {
					color: <?php print $fullNavigationActiveBackgroundColor; ?>;
				}

				#fp-nav ul li a span {
					background-color: <?php print $sectionsNavigationColor; ?>;
				}

				#fp-nav ul li a.active span {
					background-color: <?php print $sectionsNavigationActiveColor; ?>;
				}

				.material-tooltip {
					color: <?php print $tooltipColor; ?>;
				}

				.backdrop {
					background-color: <?php print $tooltipBgColor; ?>;
				}

				body {
					background-color: <?php print $fullpageColor; ?>;
				}

				#fullpage.bg-loaded:before {
					-webkit-transition: background-position <?php print $fullpageScrollingSpeed; ?>ms linear;
					-moz-transition: background-position <?php print $fullpageScrollingSpeed; ?>ms linear;
					-ms-transition: background-position <?php print $fullpageScrollingSpeed; ?>ms linear;
					transition: background-position <?php print $fullpageScrollingSpeed; ?>ms linear;
				}

				.section.bg-loaded:before {
					-webkit-transition: background-position <?php print $sectionScrollingSpeed; ?>ms linear;
					-moz-transition: background-position <?php print $sectionScrollingSpeed; ?>ms linear;
					-ms-transition: background-position <?php print $sectionScrollingSpeed; ?>ms linear;
					transition: background-position <?php print $sectionScrollingSpeed; ?>ms linear;
				}
	...
	} // END public function get_dynamic_fullpage_style
	 ...
	 * @return  void
	 */
	public function get_dynamic_section_style( $section_index = -1 ) {
		
		if( -1 === $section_index )
			$section_index = $this->current_section;
		
		$slidesNavigationColor       = $this->get_section_option( $section_index, 'slidesNavigationColor', '#333333' );
		$slidesNavigationActiveColor = $this->get_section_option( $section_index, 'slidesNavigationActiveColor', '#666666' );
		$navigationArrowsColor       = $this->get_section_option( $section_index, 'navigationArrowsColor', '#333333' );
		$navigationArrowsHoverColor  = $this->get_section_option( $section_index, 'navigationArrowsHoverColor', '#666666' );
		$sectionColor                = $this->get_section_option( $section_index, 'sectionColor', 'transparent' );

		WPFP_Helpers()->ob_start();

			?>

				.section.<?php print $this->sections[ $section_index ]->post_name; ?> .fp-slidesNav ul li a span {
					background-color: <?php print $slidesNavigationColor; ?>;
				}

				.section.<?php print $this->sections[ $section_index ]->post_name; ?> .fp-slidesNav ul li a.active span {
					background-color: <?php print $slidesNavigationActiveColor; ?>;
				}

				.section.<?php print $this->sections[ $section_index ]->post_name; ?> .fp-controlArrow.fp-prev:after,
				.section.<?php print $this->sections[ $section_index ]->post_name; ?> .fp-controlArrow.fp-next:before {
					color: <?php print $navigationArrowsColor; ?>;
				}

				.section.<?php print $this->sections[ $section_index ]->post_name; ?> .fp-controlArrow.fp-prev:hover:after,
				.section.<?php print $this->sections[ $section_index ]->post_name; ?> .fp-controlArrow.fp-next:hover:before {
					color: <?php print $navigationArrowsHoverColor; ?>;
				}

				.section.<?php print $this->sections[ $section_index ]->post_name; ?> {
					background-color: <?php print $sectionColor; ?>;
				}
	...
	} // END public function get_dynamic_section_style
