( function() {
	...
	Backbone.ModalPostsList = ( function( _super ) {
		...
		ModalPostsList.prototype.initScripts = function() {
			
			Backbone.$( '#' + this.prefix ).find( 'select' ).material_select();
			Backbone.$( '#' + this.prefix ).find( '.m_dropdown-button' ).m_dropdown();
			Backbone.$( '#' + this.prefix ).find( '.wpfp-add-post' ).on( 'click', function() {
				var $input = Backbone.$( this ).parents( 'tr' ).find( '.check-column input' );
		...
		ModalPostsList.prototype.searchChange = function( target, search ) {
			...
			var loadsLink  = thisSearch.prevAll( _this.loadsContentEl );
			...
			Backbone.$( _this.searchContentEl ).each( function() {
				...
				Backbone.$( this ).prevAll( _this.loadsContentEl ).data( 'params', params );	
