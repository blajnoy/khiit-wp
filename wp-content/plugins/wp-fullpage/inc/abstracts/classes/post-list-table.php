...
abstract class WP_Fullpage_Posts_List_Table extends WP_Posts_List_Table {
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( ! empty( $columns['cb'] ) ) {
			...
			static $cb_counter = 1;
			$columns['cb']     = '<input id="cb-select-all-' . $cb_counter . '" type="checkbox" class="' . WPFP_BBM_SELECT_ALL . '" />'
				. '<label for="cb-select-all-' . $cb_counter . '"></label>';
			...
			switch ( $column_name ) {
						...
						<th scope="row" class="check-column">
							...
							if ( $can_edit_post ) {
								...
								?>
									...
									<label for="cb-select-<?php the_ID(); ?>"></label>
				...
				case 'title':
					...
					if ( $can_edit_post && $post->post_status != 'trash' ) {
						...
						print '<a class="row-title wpfp-add-post" href="#!" title="' . esc_attr( sprintf( __( 'Add &#8220;%s&#8221;', WPFP_DOMAIN ), $title ) ) . '">' . $pad . $title . '</a>';
	...
	public function search_box( $text, $input_id ) {
		...
		?>
			<p class="search-box col s4">
				<label class="screen-reader-text" for="<?php print $input_id ?>"><?php print $text; ?>:</label>
				<a href="#" data-params='<?php print json_encode( $ajax_params ); ?>' class="btn-floating btn-flat transparent waves-effect right <?php print WPFP_BBM_LOADS_CONTENT; ?>"><i class="material-icons white-text">search</i></a>
				<input type="search" class="<?php print WPFP_BBM_SEARCH_INPUT; ?> right" id="<?php print $input_id ?>" name="s" value="<?php _admin_search_query(); ?>" placeholder='<?php print __( 'eg: my key words', WPFP_DOMAIN ); ?>' />
			</p>
		<?php
	...
	public function display_tablenav( $which ) {
		...
		?>
			<div class="tablenav col s12 <?php echo esc_attr( $which ); ?>">
	...
	public function extra_tablenav( $which = 'bottom' ) {
		...
		?>
			<div class="actions col s8">

				<a href="#" data-params='<?php print json_encode( $ajax_params ); ?>' class="btn grey lighten-2 teal-text text-darken-2 col s2 <?php print WPFP_BBM_LOADS_CONTENT; ?>"><?php _e( 'Filter' ); ?></a>
			...
			if ( is_object_in_taxonomy( $this->screen->post_type, 'category' ) ) {
				...
				$dropdown_options = array(
					...
					'class'           => "{WPFP_BBM_CAT_SELECT} col s5",
				);
	...
	public function views() {
		...
		?>
			...
			<div class="col s3">
				...
				<a class='change-view m_dropdown-button btn waves-effect waves-teal white-text left' href='#' data-activates='viewables'><?php _e( 'Choose the view', WPFP_DOMAIN ); ?></a>

				<ul id="viewables" class='m_dropdown-content'>
	...
	public function months_dropdown( $post_type ) {
		...
		?>
			<select class="<?php print WPFP_BBM_DATE_SELECT; ?> col s5" name='m'>
